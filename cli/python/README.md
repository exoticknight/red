...
This Python package and the [npm distribution](https://www.npmjs.com/package/@exoticknight/red) provide the same command contract and Skill snapshot. Choose the runtime you already use.

## Install the Skill

Requires **Python 3.11 or later** and [pipx](https://pipx.pypa.io/stable/installation/). From your project directory:
...
If you already use uv, `uvx --from red-methodology red skill install --scope repo` is another one-shot option. For a persistent command with uv, use `uv tool install red-methodology`; upgrade it with `uv tool upgrade red-methodology`.
...
```sh
pipx upgrade red-methodology
red skill status --scope repo
red skill update --scope repo
```

For one-shot use, run `pipx run --spec red-methodology red skill update --scope repo`. Replacing the runner alone does not update an installed Skill.
...
```sh
pipx run --spec red-methodology red instructions install --target AGENTS.md
pipx run --spec red-methodology red instructions export --output RED.md
```
